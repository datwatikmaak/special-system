...
class Promo:
    ...
    def _pick_random_bite(self):
        ...
        if random_bite := [x for x, _ in BITES.items() if x not in self.bites_done]:
            return random.choice(random_bite)
        else:
    ...
    def new_bite(self):
        ...
        self.random_bite = self._pick_random_bite()
        self.bites_done.add(self.random_bite)
        return self.random_bite
