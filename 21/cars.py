...
def get_all_matching_models(
    ...
    """
    grep_result = [s for val in cars.values() for s in val if grep.lower() in s.lower()]
    return sorted(grep_result)
...
def sort_car_models(cars: CarsType = cars) -> CarsType:
    ...
    sorted_cars = {key: cars[key] for key in sorted_keys}
    return {x: sorted(sorted_cars[x]) for x in sorted_cars}
